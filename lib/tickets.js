...
export async function generateTicketNumber() {
  // Try up to 5 times to generate a unique ticket number
  for (let attempt = 0; attempt < 5; attempt++) {
    const timestamp = Date.now().toString().slice(-6)
    const random = Math.floor(Math.random() * 10000).toString().padStart(4, '0')
    const ticketNumber = `TICK-${timestamp}${random}`
  ...
  // Fallback: use full timestamp + UUID-like random
  const fullTimestamp = Date.now()
  const randomLarge = Math.floor(Math.random() * 100000).toString().padStart(5, '0')
  return `TICK-${fullTimestamp}-${randomLarge}`
}
