...
// Helper function to send email via SMTP or Microsoft Graph
async function sendEmail(emailData) {
  try {
    // Check if we should use Microsoft Graph instead of SMTP
    if (process.env.EMAIL_PROVIDER === 'graph') {
      console.log('Sending email via Microsoft Graph...')
      const graphEmailService = getEmailService()

      // Convert nodemailer format to Graph API format
      const toAddress = typeof emailData.to === 'string' ? emailData.to.split(',')[0].trim() : emailData.to
      const subject = emailData.subject
      const body = emailData.html || emailData.text || ''

      // Use the generic sendEmail method from EmailService
      const result = await graphEmailService.sendEmail(toAddress, subject, body)
      return result
    }
...
function getTicketUrl(ticketId) {
  const baseUrl = process.env.BASE_URL || process.env.NEXT_PUBLIC_BASE_URL || 'https://helpdesk.surterreproperties.com:3011'
...
export async function sendTicketCreatedEmail(ticket, requester) {
  try {
    ...
    // Add a small delay to allow attachments to be uploaded first
    await new Promise(resolve => setTimeout(resolve, 2500))

    // Query for attachments using the shared Prisma client
    ...
    // Get all email addresses for the user (primary + alternates)
    const { getAllUserEmails } = await import('./user-email-utils.js')
    const allEmails = await getAllUserEmails(requester.id)

    if (allEmails.length === 0) {
      allEmails.push(requester.email) // Fallback to primary email
    }
    ...
    if (attachments.length > 0) {
      const attachmentList = attachments.map(att =>
        `<li style="padding: 8px 0; border-bottom: 1px solid #e0e0e0;">
          ...
          <span style="color: #666; font-size: 0.9em;">(${formatFileSize(att.fileSize)})</span>
        </li>`
      ).join('')

      attachmentsHtml = `
        <div style="background-color: #f0f8ff; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #007bff;">
          <p style="margin-top: 0;"><strong>📎 Attachments (${attachments.length}):</strong></p>
          <ul style="list-style: none; padding: 0; margin: 10px 0 0 0;">
            ${attachmentList}
          </ul>
          <p style="margin-bottom: 0; font-size: 0.9em; color: #666; margin-top: 10px;">
            <em>Click "View Ticket in Helpdesk" below to access and download your attachments.</em>
          </p>
        </div>
      `
    }
    ...
    // Send to all user's email addresses
    await sendEmail({
      from: getFromAddress(),
      to: allEmails.join(', '), // Send to all emails
      subject,
      html,
      headers: {
    ...
    console.log(`✅ Sent ticket creation email to ${allEmails.length} email(s): ${allEmails.join(', ')}`)
    if (attachments.length > 0) {
...
export async function sendTicketCommentEmail(ticket, comment, user, requester, assignee = null) {
  try {
    ...
    // Get all email addresses utility
    const { getAllUserEmails } = await import('./user-email-utils.js')

    // Send emails to all recipients
    for (const recipient of recipients) {
      // Get all email addresses for this recipient
      const allEmails = await getAllUserEmails(recipient.user.id)
      if (allEmails.length === 0) {
        allEmails.push(recipient.user.email) // Fallback to primary email
      }
      ...
      console.log(`✅ Sent comment email to ${allEmails.length} email(s): ${allEmails.join(', ')}`)
    }
