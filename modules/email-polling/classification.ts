...
function extractHeuristicSignals(email: EmailToClassify): HeuristicSignals {
  ...
  const patterns = {
    ...
    bounce: /^undeliverable:|delivery status notification|mail delivery failed|undeliverable|returned mail|message could not be delivered|delivery failure|nondelivery notification|not delivered|bounce|postmaster@|mailer-?daemon@/i,
  ...
  return {
    ...
    bounce: patterns.bounce.test(senderEmail) || patterns.bounce.test(subject) || patterns.bounce.test(body),
