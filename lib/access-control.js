...
export async function hasTicketAccess(user, ticket) {
  ...
  if (isManager || isStaff) {
    ...
    const userDepartmentIds = userWithDepartments?.departments?.map(ud => ud.department.id) || []

    // Check if ticket's assigned department matches user's departments
    if (ticket.departmentId && userDepartmentIds.includes(ticket.departmentId)) {
      return true
    }
  }
...
export async function buildTicketAccessWhere(user) {
  ...
  if (isManager || isStaff) {
    ...
    if (isManager) {
      ...
      // 2. All tickets routed to their departments (via AI decisions)
      return {
        OR: [
          { requesterId: user.id },
          { assigneeId: user.id },
          {
            aiDecision: {
              finalDepartment: { in: userDepartmentIds }
            }
          }
        ]
      }
