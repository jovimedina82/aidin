...
import { sendEmailViaGraph } from './graph-email.js'

// Helper function to send email via Graph API or SMTP
async function sendEmail(emailData) {
  try {
    ...
    if (transporter) {
      ...
      throw new Error('Both Graph API and SMTP email sending failed')
    }
  }
}
...
function getTicketUrl(ticketId) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://helpdesk.surterreproperties.com'
